...
export const metadata: Metadata = {
  ...
  openGraph: { title: "eCommerce Store Management | Unified Branding Experts", description: "Delegate store upkeep with a defined monthly scope. Plans from $249/month.", url, images: [`${url}/opengraph-image`] },
};

export default function StoreManagementServicePage() {
  const visibleFaqs = STORE_MANAGEMENT_FAQS.filter((_, i) => [0, 1, 2, 4, 5].includes(i));
  ...
  return <main className="pt-32 pb-24 px-6 md:px-12 max-w-7xl mx-auto space-y-14 text-[#161616]">
    <FaqSchema faqs={visibleFaqs.map(({ question, answer }) => ({ q: question, a: answer }))} />
    ...
    <section className="space-y-5"><h2 className="font-display text-3xl font-bold">Common store management questions</h2>{visibleFaqs.map(faq => <details key={faq.question} className="border border-[#E0DDDB] rounded-2xl p-5"><summary className="font-semibold cursor-pointer">{faq.question}</summary><p className="mt-3 text-[#585858] leading-relaxed">{faq.answer}</p></details>)}</section>
    <p className="text-[#585858]">Planning a new store first? Explore <Link className="underline text-[#6B46C1]" href="/ecommerce-growth-packages">eCommerce setup packages</Link>. For search visibility beyond listing upkeep, see <Link className="underline text-[#6B46C1]" href="/services/ai-seo-agency">AI SEO services</Link>.</p>
  </main>;
}
